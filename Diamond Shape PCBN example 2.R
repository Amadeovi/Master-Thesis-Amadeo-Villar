...
margins <- BiCopSim(N, RVMcop.estcop.to$family[3,2], RVMcop.estcop.to$par[3,2],
...
ggplot(data, aes(x = U2, y = U3))+geom_point(size=size.dot)+
  labs(title = TeX(paste("True margins","$U_{2}$","and","$U_{3}$",
                         ",","$\\tau$","=",round(RVMcop.estcop.to$emptau[3,2],3))),
...
ggplot(margins, aes(x = margins[,1], y = margins[,2]))+geom_point(size=size.dot)+
  labs(title = TeX(paste("Simulated data from","$\\hat{C}_{2,3}$",
                         ":",family.list[as.character(RVMcop.estcop.to$family[3,2])],",",
                         "$\\tau$","=",round(RVMcop.estcop.to$emptau[3,2],3))), 
       x = TeX("$U_{2}$") , y = TeX("$U_{3}$"))+theme(plot.title = element_text(hjust = 0.5))

# We also compute a goodness of fit test for these data:
BiCopGofTest(databbn1[,2],databbn1[,3],family=RVMcop.estcop.to$family[3,2],
             par=RVMcop.estcop.to$par[3,2],par2=RVMcop.estcop.to$par2[3,2],
...
##############################
# Let's apply the hill climbing algorithm to our data, but
# first we need to transform the margins to gaussian
